...
let workList: WorkState[] = [];

const Work: FC = () => {
  const [works, setWorks] = useState(workList);
  const [filterWork, setFilterWork] = useState(workList);
  const [activeFilter, setActiveFilter] = useState("All");
  const [animateCard, setAnimateCard] = useState<AnimateCardState>({
    y: 0,
    opacity: 1,
  });
  ...
  const handleWorkFilter = (item: string) => {
    ...
    setTimeout(() => {
      ...
      if (item === "All") {
        ...
        setFilterWork(
          works.filter((work: WorkState) => work.tags.includes(item))
        );
      }
    }, 500);
  };
